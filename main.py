...
def vk_yadisk_parser(VK_TOKEN, YA_TOKEN, VK_ID, folder='VK_Photos'):
    success = 0
    result = []
    ...
    for item in dict['response']['items']:
        ...
        ya.upload_url_to_disk(f'{folder}/{good_look_date} - {item["likes"]["count"]} likes.jpg', item['sizes'][-1]['url'])
        result.append({'file_name': f'{good_look_date} - {item["likes"]["count"]} likes.jpg', 'size': f'{item["sizes"][-1]["type"]}'})
        success += 1
        print(f'{success} из {counter}')
        if success >= quantity:
            break
